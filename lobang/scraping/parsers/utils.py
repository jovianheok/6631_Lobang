"""
Purpose: Hold reusable helper functions for text cleaning, normalization, and shared parsing logic.
"""
...
def parse_day_month_from_text(text: str, post_date: date) -> Optional[date]:
    """
    Purpose: Parse a `DD Mon` style date and normalize it to YYYY-MM-DD.
    Uses the post year by default, rolling forward one year when needed.
    """
    ...
    try:
        candidate = date(post_date.year, month, day)
    except ValueError:
        return None
